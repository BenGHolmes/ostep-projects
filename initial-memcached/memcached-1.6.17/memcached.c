 ...
 * returns a response string to send back to the client.
 */
enum delta_result_type do_multiply_by(conn *c, const char *key, const size_t nkey,
 ...
 * returns a response string to send back to the client.
 */
enum delta_result_type do_divide_by(conn *c, const char *key, const size_t nkey,
                                    const int64_t delta, char *buf, uint64_t *cas,
                                    const uint32_t hv, item **it_ret)
{
    char *ptr;
    ...
    else
    {
        ...
        if (settings.verbose)
        {
            fprintf(stderr, "Tried to do div on invalid item\n");
        }
        if (it->refcount == 1)
            do_item_remove(it);
        return DELTA_ITEM_NOT_FOUND;
