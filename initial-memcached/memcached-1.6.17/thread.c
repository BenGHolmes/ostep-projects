...
struct conn_queue
{
    STAILQ_HEAD(conn_ev_head, conn_queue_item)
    head;
                                   ...
                                   char *buf, uint64_t *cas)
{
    ...
    hv = hash(key, nkey);
    item_lock(hv);
    ret = do_multiply_by(c, key, nkey, b, buf, cas, hv, NULL);
    item_unlock(hv);
    return ret;
}

/*
 * Does division on a numeric item value.
 */
                                 ...
                                 char *buf, uint64_t *cas)
{
    ...
    hv = hash(key, nkey);
    item_lock(hv);
    ret = do_divide_by(c, key, nkey, b, buf, cas, hv, NULL);
    item_unlock(hv);
    return ret;
}
