...
static void process_multiply_command(conn *c, token_t *tokens, const size_t ntokens)
{
    ...
    switch (multiply_by(c, key, nkey, delta, temp, NULL))
    {
    case OK:
        out_string(c, temp);
        break;
    case NON_NUMERIC:
        out_string(c, "CLIENT_ERROR cannot multiply by non-numeric value");
        break;
    case EOM:
        out_of_memory(c, "SERVER_ERROR out of memory");
        break;
...
static void process_divide_command(conn *c, token_t *tokens, const size_t ntokens)
{
    char temp[INCR_MAX_STORAGE_LEN];
    uint64_t delta;
    char *key;
    ...
    switch (divide_by(c, key, nkey, delta, temp, NULL))
    {
    case OK:
        out_string(c, temp);
        break;
    case NON_NUMERIC:
        out_string(c, "CLIENT_ERROR cannot divide by non-numeric value");
        break;
    case EOM:
        out_of_memory(c, "SERVER_ERROR out of memory");
        break;
...
void process_command_ascii(conn *c, char *command)
{
    ...
    else if (first == 'd')
    {
        ...
        {
            WANT_TOKENS_OR(ntokens, 4, 5);
            process_divide_command(c, tokens, ntokens);
        }
        else
        {
            out_string(c, "ERROR");
