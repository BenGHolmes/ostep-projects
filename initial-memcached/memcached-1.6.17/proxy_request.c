...
{
    if (pr->tokens[pr->ntokens] - pr->tokens[pr->ntokens - 1] >= NOREPLYLEN && strncmp(NOREPLYSTR, pr->request + pr->tokens[pr->ntokens - 1], NOREPLYLEN) == 0)
    {
        pr->noreply = true;
    }
    return 0;
...
{
    // BEN_TODO: Add `mult` and `div` requests to this function. Copy `incr` format
