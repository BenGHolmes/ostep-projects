...
struct mcp_backend_s
{
    ...
    STAILQ_ENTRY(mcp_backend_s)
    be_next;            // stack for backends
...
struct _io_pending_proxy_t
{
    ...
    STAILQ_ENTRY(_io_pending_proxy_t)
    io_next;                 // stack for backends
...
struct mcp_pool_s
{
    ...
    proxy_ctx_t *ctx;     // main context.
    STAILQ_ENTRY(mcp_pool_s)
    next; // stack for deallocator.
